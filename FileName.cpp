...
typedef struct
{
	...
	string dob; //Store user dob
	string contactnumber; //Store user contact number
	//string age; //Store user age
	string address; //Store user address
	string test_results; //Store user test results
}USER; //Yu_xiang part
...
/*string structToString(Date s) {
	ostringstream oss;
	oss << s.day << s.month << s.year;
	return oss.str();
}*/
...
int main(void)
{
	...
	cout << "1.User \n";
	cout << "2.New User \n";
	cout << "Choose type of user by input a digit: ";
	cin >> num;
	if (num == 1)
	{
		USER user;
...
void check_login(USER& user) //Function to check if the entered username and password match
{
	...
	else
	{
		vector<USER> users;
		USER user;
		while (in_file >> user.name >> user.useric >> user.dob >> user.contactnumber >> user.address >> user.username >> user.password)
		{
...
void user_registration(USER& user)
{
	// Get user name
	string name;
	cout << "Name: ";
	...
	USER user = { name, ic, dob, contactnumber, address, username, password};
	//USER user = { name, ic, dob, contactnumber, username, password };
...
void write_user_data(USER user)
{
	...
	{
		out_file << "\n" << user.username << " " << user.password << " " << user.dob << " "  << user.contactnumber << " " << user.address << " " << user.test_results; "\n";
		out_file.close();
	}
}
...
void user_menu(USER user)
{
	...
	do
	{
		...
		case 1:
			...
			cout << "Day of birth: " << user.dob << endl;
...
//HIENG
void displayMenu() // DISPLAY THE MENU FOR THE USER 
{
...
{// admin assign c19 categories into user status respectively
	...
	1.  type in users full name (later convert to lower case)
	1.1 READ inputed full name and  from a respective COVID_19 category file and save it to a temp variable called CATEGORY
	2.  analyse CATEGORY variable
